/**
 * Shared mock mine data (Newman Iron Operations - Multi-Structure Mine Site).
 *
 * Single source of truth for the procedurally-generated demo data used both by
 * the visualization endpoint (`api/levels/current.js`) and, as a fallback, by
 * the AI grounding layer when no database is configured. Keeping them in sync
 * ensures the AI "sees" exactly what is rendered on screen.
 */

/**
 * Get mock levels for the Main Open Pit structure
 */
export function getPitMainLevels(timestamp) {
 ...
 */
export function getDeclineNorthLevels(timestamp) {
    const hour = timestamp.getHours();
    ...
    return [
        ...
        },
        {
            ...
            riskScore: 45,
            riskBand: 'medium',
            riskExplanation: 'MEDIUM risk: Primary ventilation with air quality monitoring.',
            triggeredRules: [
 ...
 */
export function getProcessingLevels(timestamp) {
    return [
        {
            level: 1,
            ...
            riskScore: 32,
            riskBand: 'medium',
            riskExplanation: 'MEDIUM risk: Heavy equipment operation with rotating machinery.',
            triggeredRules: [
 ...
 */
export function getMockStructures(timestamp) {
 ...
 */
export function flattenToLegacyLevels(structures) {
    const levels = [];
...
/**
 * Build a query-grounding context from mock data, shaped like the DB rows that
 * `gatherQueryContext` expects. Used as a fallback when no database is
 * configured so the natural-language assistant is grounded in the same mine
 * state that is rendered on screen.
 *
 * @param {Date} timestamp - Reference time (drives time-varying risk)
 * @returns {{levels: Array, events: Array, measurements: Array, alerts: Array}}
 */
export function buildMockQueryContext(timestamp) {
    const structures = getMockStructures(timestamp);
    const flat = flattenToLegacyLevels(structures);

    // Levels shaped like snapshot_levels rows
    const levels = flat.map(l => ({
        level_number: l.level,
        level_name: `${l.structureName} — ${l.name}`,
        risk_score: l.riskScore,
        risk_band: l.riskBand,
        risk_explanation: l.riskExplanation
    }));

    // Derive active alerts from high-risk levels (risk >= 70), mirroring how the
    // alerting layer would surface them.
    const alerts = flat
        .filter(l => l.riskScore >= 70)
        .map(l => ({
            level_number: l.level,
            cause: (l.triggeredRules[0]?.reason) || l.riskExplanation,
            risk_score: l.riskScore,
            status: 'active'
        }));

    return { levels, events: [], measurements: [], alerts };
}
