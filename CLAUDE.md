...
- Guided tour: v3/js/tour/ (22 steps, 8 chapters) drives the REAL UI via
  ...
  pre-generated by v3/tools/gen-tour-audio.mjs against POST /api/v3/tts
  (OpenRouter gpt-audio-mini streams pcm16 → WAV → local ffmpeg → mp3;
  the requested Gemini TTS model is not served by OpenRouter yet). After
  editing any step's `narration`, RERUN the generator — text and audio
  drift silently otherwise. The tour must never block the console: hooks
  are try/caught, missing audio falls back to timers.
