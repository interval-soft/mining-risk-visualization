 ...
 *                KEEP IN SYNC: rerun the generator after editing narration.
...
export const TOUR_STEPS = [
    ...
    },
    {
        ...
        target: '#map',
        title: 'The real site',
        html: `You are looking at the actual construction site, south-west of the Padeswood
               cement works. The plot plan overlay is the project's own <b>Appendix H —
               Construction Work Areas</b> drawing, georeferenced against the planning
    ...
    },
    {
        ...
        target: '.map-toolbar',
        title: 'Map controls',
        html: `<b>Plot plan</b> toggles the Appendix H drawing. <b>CWA zones</b> brings the
    ...
    },
    {
        ...
        narration: `The console runs on UK site time, and the demo scenario anchors itself to today's seven a m shift start. Every validity countdown, revalidation deadline and expiry you see is live, whenever you open the console.`
    },
    ...
    },
    {
        ...
        narration: `The permit board lists every permit on site, grouped by lifecycle state: issued, authorised awaiting acceptance, in approval, suspended, and recently closed. The colour chip is the permit type, using the same colour coding as the paper forms. Live permits show a countdown to expiry.`
    },
    {
        id: 'validity',
        chapter: 'Permit board',
        ...
        narration: `Each permit type carries its maximum validity from section four point one. Hot work, for example, is limited to a shift and must be revalidated daily, up to seven days. The system computes the validity window automatically; nobody gets to type their own expiry date.`
    },
    ...
    // ---------------- Chapter 3 — Lifecycle ----------------
    {
        ...
        html: `PTW-0133 — a confined-space entry inside the absorber. The detail shows the
               ...
               system says so.`,
        narration: `Here is the anatomy of a permit. P T W zero one three three is a confined space entry inside the absorber. The detail panel shows the full signature chain: work applicant, area authority, permit issuer, then performing authority, as required by section six point two point one. Below it, the document checklist and the validity window. A permit with missing documents is invalid, and the system says so.`,
    ...
    },
    {
        ...
        narration: `This permit has just been requested. As area authority you are offered exactly one action: review. Feel free to try it. Every transition is validated twice, in the browser and on the server, against the section five state machine. Illegal moves are rejected with the procedure section that forbids them, and suspensions demand a written reason.`,
    ...
    },
    {
        ...
        narration: `Here is the same conflict drawn on the ground: pulsing rings around both work fronts, and a dashed interaction line. Today's high severity case is a welder and a confined space crew in the same absorber, zero metres apart. The system detects this before anyone lights a torch, not after.`,
        before: async (app) => {
    ...
    // ---------------- Chapter 6 — Isolations ----------------
    {
        ...
        narration: `Section eight of the procedure covers energy isolations. Each isolation confirmation certificate lists its isolation points with the standard tag colours: red for process, green for electrical, blue for instrument. It tracks the lockbox and key holder, and the permits that depend on the isolation. Isolation points are pinned on the map, like permits.`,
    ...
    },
    {
        ...
        narration: `We just asked the assistant whether hot work can start in area zero six two zero right now. The live answer arrives in a couple of seconds. Expect a hard no, on three grounds from the procedure: an expired hot work permit, an incomplete approval chain, and the live simops conflict in that very area.`,
        before: async (app) => {
